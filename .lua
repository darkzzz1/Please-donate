...
if isfile and writefile and typeof(isfile) == 'function' and typeof(writefile) == 'function' then
	if not isfile('PromptedDiscordCFCommunity.txt') then
		...
		Module.Prompt({
			invite = "https://discord.gg/YDgdjt2E",
			name = "BlooHub",
		})
	end
end
...
Players.LocalPlayer.leaderstats.Raised.Changed:Connect(function()
	...
	if getgenv().settings.webhookToggle and getgenv().settings.webhookBox then
		...
		webhook("💰 Tip | Amount: " .. tostring(Players.LocalPlayer.leaderstats.Raised.Value - RaisedC) .. 'R$ (after tax: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value - RaisedC * 0.6) .. 'R$) | Total: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value) .. 'R$ | Account: ' .. Players.LocalPlayer.DisplayName .. ' (' .. Players.LocalPlayer.Name .. ')')
	end
		...
		if (plrChatted.Character and plrChatted.Character.Humanoid.RootPart) then
			...
			if (root.Position - game:GetService('Players').LocalPlayer.Character.Humanoid.RootPart.Position).Magnitude < 11 then
				if message == 'hello' or message == 'hi' or message == 'sup' then
					messageRequest:FireServer("hello " .. game:GetService('Players')[speaker].DisplayName, 'All')
...
if game:GetService("CoreGui").imgui.Windows.Window.Title.Text == "Loading..." then
	game:GetService("CoreGui").imgui.Windows.Window.Title.Text = "Bloo Hub | bloo6666"
end
